...
for (name in names) {
  ...
  full_selections <- rbind(full_selections, group_selections)
...
full_selections$Player <- c(NA, rep(names, each = 48))
...
full_selections_result$Game <- paste0(full_selections_result$Game,
                                      " (", full_selections_result$Result,
                                      ")")
...
full_selections_result  %>%
  select("Player", "Prediction", "Points") %>% 
  kbl(row.names = FALSE) %>%
  kable_styling() %>%
  pack_rows(index = 
              table(gsub("[1-9].*", "", full_selections_result$Game))[order(
                order(unique(cbind(full_selections_result$`Home Team`,
...
full_selections_fixtures  %>%
  ...
  pack_rows(index = 
              table(gsub("[1-9].*", "", full_selections_fixtures$Game))[order(
                order(unique(cbind(full_selections_fixtures$`Home Team`,
